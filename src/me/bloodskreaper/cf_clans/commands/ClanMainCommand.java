		...
		if (args.length > 1) {
			...
			if (CF_Clans.getClanManager().getClanOfMember(p.getUniqueId()) == null) {
				...
				p.sendMessage(ChatColor.translateAlternateColorCodes('&',
						"§8[§f" + c.getClanDisplayName() + "§8] §f" + c.getName()));
				String role = "";
			...
			if (CF_Clans.getClanManager().getClanOfMember(p.getUniqueId()) != null
					&& CF_Clans.getClanManager().getClanOfMember(p.getUniqueId()).getLeader().equals(p.getUniqueId())) {
				Clan c = CF_Clans.getClanManager().getClanOfMember(p.getUniqueId());
				p.sendMessage("§aEinladungen deines Clans:");
				if (CF_Clans.getInviteManager().getInvitesOfClan(c).size() > 0) {
					p.sendMessage(InviteListToCommaStringClan(CF_Clans.getInviteManager().getInvitesOfClan(c)));
				} else {
					p.sendMessage("§cKeine Einladungen vorhanden.");
				}
			}
